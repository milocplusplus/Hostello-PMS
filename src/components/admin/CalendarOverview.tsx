...
export function CalendarOverview({
  ...
  return (
    <div className="card divide-y divide-border-hairline">
      <div className="flex flex-col sm:flex-row sm:items-end gap-1 sm:gap-4 px-4 pt-3 pb-2">
        <p className="sm:w-56 shrink-0 text-[10px] uppercase tracking-wider text-ink-muted">
          Client
        </p>
      ...
      {clients.map((c) => {
        ...
        return (
          <Link
            key={c.id}
            href={c.href}
            className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 px-4 py-3 hover:bg-surface-2/40 transition-colors"
          >
            <div className="sm:w-56 shrink-0 flex items-center gap-2.5 min-w-0">
              <Avatar name={c.name} size={28} rounded="lg" />
              <div className="min-w-0">
                <p className="text-sm text-ink-primary truncate">{c.name}</p>
                <p className="text-[11px] text-ink-muted truncate">
                  {c.properties} {c.properties === 1 ? "property" : "properties"} · {pct}% booked
              ...
              {days.map((d, i) => {
                ...
                return (
                  <span
                    ...
                    style={{ backgroundColor: shade(ratio, isWeekend(d)) }}
                  />
                );
              })}
            </div>
