...
export default async function CalendarPage({
  ...
  // Group properties by client for sub-headers.
  const groups: { clientName: string; properties: typeof properties }[] = [];
  for (const p of properties) {
    const clientName = (p.clients as unknown as { name: string } | null)?.name ?? "—";
    let group = groups.find((g) => g.clientName === clientName);
    if (!group) {
      group = { clientName, properties: [] };
      groups.push(group);
    }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="card p-4 flex items-center justify-between">
        <Link
          href={`/admin/calendar?month=${formatMonthParam(prevYear, prevMonth0)}`}
        ...
        <Link
          href={`/admin/calendar?month=${formatMonthParam(nextYear, nextMonth0)}`}
          className="p-1.5 rounded-md text-ink-secondary hover:text-ink-primary hover:bg-surface-2 transition-colors"
        >
      ...
      <div className="card p-3 overflow-x-auto">
        <table className="border-separate" style={{ borderSpacing: "2px" }}>
          <thead>
            <tr>
              <th className="sticky left-0 bg-surface-1 z-10 text-left text-[10px] text-ink-muted font-normal px-2 pb-2 min-w-[160px]">
                Property
              </th>
              {days.map((d) => (
                <th
              ...
              <Fragment key={group.clientName}>
                <tr>
                  <td
                    ...
                    className="sticky left-0 bg-surface-1 text-[11px] font-medium text-ink-secondary pt-3 pb-1"
                  >
                    {group.clientName}
                  </td>
                </tr>
                      ...
                      if (status.kind === "available") {
                        return (
                          <td key={d} className="p-0">
                            <Link
                              href={`/admin/bookings/new?property=${p.id}&date=${d}`}
                              title="Add booking"
