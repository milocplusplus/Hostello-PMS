...
export default async function AdminCheckInsPage() {
  ...
  const { data } = await supabase
    .from("bookings_v")
    .select(
      "id, guest_name, guest_phone, guests_count, check_in, check_out, source, status, sale_price, checked_in_at, checked_out_at, is_short_stay, short_stay_start, short_stay_end, expected_arrival, clients:clients_v(name), booking_properties(properties:properties_v(name))"
    )
