    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        {/* Owner only: `bookings_v` blanks `client_payout` for ops, so an ops
            ...
            every client is not a statement either — hence the prompt rather
            than a button that would build the wrong document. */}
        {showMoney && !searching && (
          statementReport ? (
            <StatementPdf
            ...
            <p className="text-[11px] text-ink-muted max-w-[14rem] text-right">
              Pick a client to export their statement for this month.
            </p>
          )
        )}
      </div>
